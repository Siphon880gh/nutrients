...
**Guided DOM** — `ensureDayGuidedEl` injects `.day__guided` (list + **Add food** + **Add Others** + **Rearrange**). Rearrange shows a drag handle on each line; a short click on the handle (press and release within `GUIDED_HANDLE_SELECT_MS`) toggles multi-select (`.day__food-item--selected`); dragging any row moves the selected set plus the dragged row as a block (`reorderGuidedEntries`) with before/after/end drop indicators. After a drag ends, `#guided-finish-popover` (**Finish Rearrange?**) appears at the pointer; click it to exit that day’s rearrange, or it hides if the pointer moves past `GUIDED_FINISH_MOVE_PX` (and is not over the popover) or the page/list scrolls. **Remove** (×): a short click asks confirm and deletes that line. Click-and-hold × (~`GUIDED_REMOVE_HOLD_MS`) enters multi-select delete (`.day__guided--remove-select`): that line’s × stays visually on (`.day__food-item-remove--on` / `.day__food-item--remove-selected`); while in this mode, clicking × on any line toggles selection. Footer **Confirm Deletion** (dominant, above Cancel; `[data-action="confirm-guided-remove-select"]`) deletes the selected lines and exits the mode. **Cancel** (`[data-action="cancel-guided-remove-select"]`) exits without deleting. Escape also cancels. Still persists as newline text.
