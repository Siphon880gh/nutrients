...
- **TDEE calculator** — `#tdee-calculator-modal`: Mifflin–St Jeor BMR × activity factor; resistance (days/week or weekly heavy/light sets) + optional cardio (days/week, minutes/session, intensity light→brisk/incline→moderate→hard→vigorous; 30 min = baseline). Inputs persist in settings `tdeeCalc` (saved while editing / on close); **Reset** clears `tdeeCalc` to null and blanks the form; **Use this value** writes settings `tdee` (+ body weight when set).
