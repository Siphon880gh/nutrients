...
| Settings / TDEE / weight | `openSettingsModal`, `loadTdee`, `saveTdee`, `getTdee`, `getTdeeBaseline`, `loadBodyWeight`, `saveBodyWeight`, `getBodyWeightKg`, `settingsWeightKgFromInput`, `syncSettingsWeightInput`, `setSettingsWeightUnit`, `readSettingsWeightFromInput`, `openTdeeCalculatorModal`, `saveTdeeCalcInputs`, `resetTdeeCalcInputs`, `calcMifflinStJeor`, `openTdeeHintModal`, `openTdeeHrZonesModal`, `updateTdeeHrZonesHighlight`, `openTdeeRpeModal`, `openMacroSplitHintModal`, `renderMacroSplitCarousel`, `MACRO_BODY_TYPES` |
| Definition modals | `openMicroDefModal`, `renderMicroDefBody`, `openLongevityDefModal`, `renderLongevityDefBody`, `setMicroDefFullscreen`, `setDefModalReturnSources`, `returnFromDefModalToSources`, `microDefConditionSectionHtml`, `vitaminAKeyDifferencesHtml`, `vitaminKKeyDifferencesHtml`, `fiberBulkingTypeHtml`, `insolubleToSolubleFiberCompareHtml`, `loadMicroDefinitions`, `loadLongevityDefinitions`; long food-note reader `openFoodNoteModal` / `closeFoodNoteModal` / `foodNoteBodyHtml` |
...
| Highlights / editor modes | `updateDayHighlights`, `highlightedHtml`, `highlightedDayHtml`, `highlightServingMultipliersHtml`, `setDayEditorMode`, `updateDayEditorMode`, `backdropCaretRect`, `caretIndexFromBackdropPoint`, `setDayInputSelection`, `dayHighlightsEnabled` + `loadDayHighlightsPreference`/`saveDayHighlightsPreference`/`setDayHighlightsEnabled`/`syncDayHighlightsToggleUi`, `dayWordWrapEnabled` + `loadDayWordWrapPreference`/`saveDayWordWrapPreference`/`setDayWordWrapEnabled`/`syncDayWordWrapToggleUi` (body `day-word-wrap-off`), `refreshAll`, `syncScroll` |
...
| Multi-week diary | `dayMealsByDate`, `viewedWeekStart`, `EARLIEST_DIARY_DATE` (`2026-05-01`) / `earliestWeekMondayKey` / `clampWeekMondayKey`, `toDateKey` / `parseDateKey` / `parseTypedDate` / `mondayOf` / `addDays` / `weekDateKeys` / `formatDayDateLabel` / `formatWeekRangeLabel`, `flushEditorsToDayMeals` / `loadEditorsFromDayMeals`, `setViewedWeekStart` / `stepViewedWeek` / `goToThisWeek` / `openWeekJumpModal` / `#week-jump-modal` / `updateWeekNavUi` / `updateDayDateLabels`, diary food search via `openDiarySearchModal` / `renderDiarySearchResults` / `jumpToDiarySearchDate` / `#diary-search-modal`, `copyDayToDateKey` / `copyWeekToMondayKey` / `copyDayToToday` / `copyDayToYesterday` / `copyDayToTomorrow` / `copyViewedWeekToThisWeek` / `#copy-date-modal` / `handleDayCopyAction` / `updateCopyActionButtons`, `viewedWeekStart` (settings) |
...
- **Weekly average view** (default): average daily amount = week sum ÷ `weekAverageDayCount()` — **7** for past or future viewed weeks; for the **current** calendar week, Mon through today’s blue column (inclusive). **% target** comes from `microNutrientTargetPct(key, avgDaily)`, which picks the first available reference: **FDA % DV** (`dailyDv` → `getDailyMicroDv`, e.g. female iron 18 mg, male 8 mg), else **IOM body-weight minimum** (`iomBwMinPct`, amino acids; needs body weight set), else **study min** (`STUDY_MIN_MICRO_REFS`; `limiting: false`, higher % = better — e.g. glycine 10 g/day for collagen synthesis), else **study max** (`STUDY_MAX_MICRO_REFS`; `limiting: true` so high = red), else unscored (`NO_STANDALONE_REF_MICRO_KEYS` / no DV): `%` slot shows bare intake via `formatNoTargetAmountText` (no `%`, muted `--amount` class) instead of `—` (same for longevity rows with no DV). Rendered by `renderMicroWeeklyList` into `#dashboard-micro-list`.
...
- **Condition focus / intake filter** (`MICRO_CONDITION_FOCUS` + `MICRO_INTAKE_FILTER`, `#dashboard-micro-condition-toggle`): filters rows to condition-relevant micros (+ optional `longevityNutrients` such as `creatine`, `saturatedFat`, `glycemicIndex` for **Hair loss**), or to poorly-/well-absorbed sets (`poorlyAbsorbed` uses `microRequiresDailyIntake`). **Nutrition Intake** group also includes **American Common Deficiencies** (`americanCommonDeficiencies`: A, D, Ca, Mg, iodine), **Fat-soluble vitamins** (`fatSolubleVitamins`: A, D, E, K), and **B vitamins** (`bVitamins`: B1 thiamine, B2 riboflavin, B3 niacin, B5 pantothenic acid, B6, B7 biotin, B9 folate, B12 cobalamin). Adds a **Focus:** section at top of explain modals when JSON has matching condition key. Session-only condition id (not persisted); **weekly/daily view** and **Daily Targets** are persisted (settings `microViewDaily` / `microShowDv`). Condition-specific tip asides: `#micro-tip-caffeine` (hidden when any filter active), `#micro-tip-cataracts`, `#micro-tip-hair-loss` / `#micro-tip-hair-loss-herbs` / `#micro-tip-hair-loss-prescriptions`, `#micro-tip-common-deficiencies` (stays for American Common Deficiencies), `#micro-tip-fat-soluble` (stays for Fat-soluble vitamins), `#micro-tip-b-vitamins` (stays for B vitamins). Choosing a condition focus clears sticky Filter (incl. By Nutrients); choosing sticky Filter clears condition focus.
...
- **By Nutrients** (nested under Filter on both sticky bars): disclosure `.dashboard__nutrient-filter`; presets `NUTRIENT_FILTER_PRESETS` (`common-deficiencies`, `fat-soluble`, `b-vitamins`) via `applyNutrientFilterPreset`; typeahead combobox (`[data-nutrient-filter-input]` / `nutrientFilterSuggestMatches`) adds via `addStickyNutrientFilter`; chips remove via `removeStickyNutrientFilter`. Keys limited to `MICRO_ALL_FIELDS` (`normalizeFilterStickyNutrientKeys`). Persisted as JSON array under settings `filterNutrients`.
...
**Vitamin A breakdown** — total `vitaminA` keeps FDA % DV (900 mcg RAE male / 700 female). Optional subforms `vitaminARetinol` (mcg retinol) and `vitaminABetaCarotene` (mcg β-carotene mass) live in `micros`. Retinol is scored with `STUDY_MAX_MICRO_REFS` at the IOM adult UL (3,000 mcg/day preformed A only). β-Carotene is in `NO_STANDALONE_REF_MICRO_KEYS` (no separate DV/RDA/ratio). Explain modals append `vitaminAKeyDifferencesHtml`. When `longevity.carotenoids` is unset, `resolveLongevityValue` prefers `micros.vitaminABetaCarotene` (÷ 1000 → mg) before estimating from total `vitaminA`.

**Omega-3 / Omega-6** — `omega3`, `epa`, `dha`, and `omega6` live in `MICRO_FIELDS` (g) and bridge to longevity via `LONGEVITY_KEYS_ALSO_IN_MICRO` (numeric in `micros`, `true` in `longevity`). Labels: Omega-3 (total), Omega-3 (EPA), Omega-3 (DHA), Omega-6 (total). Sex-specific IOM AIs in `demographic-dv.js` (omega3 1.6/1.1 g; omega6 17/12 g). `omega6` is limiting via `LONGEVITY_MICRO_LIMITING_KEYS`. EPA/DHA are `NO_STANDALONE_REF_MICRO_KEYS` (unscored forms). Explain copy in `omega3KeyDifferencesHtml`. Micro tip `#micro-tip-omega` covers EPA∶DHA ratio guidance. Run Analysis week/day includes them under the Omega fatty acids section (`MICRO_ANALYSIS_SECTIONS` / `microAnalysisSectionIdsForEntry`).
...
**Carotenoids** — stored under `longevity.carotenoids` (mg); DV 15 mg/day. When not set on a food, `resolveLongevityValue` estimates mg from `micros.vitaminABetaCarotene` (÷ 1000) when present, else from `micros.vitaminA` when vit A > 50 mcg (÷ 100). Ranked sources and panel totals use `resolveLongevityValue`, not raw `kw.longevity.carotenoids`.
...
- **`openMicroDefModal(key)` / `renderMicroDefBody`** — optional **Focus:** section from `microDefConditionSectionHtml`; soluble/insoluble fiber rows append `fiberBulkingTypeHtml`; vitamin A retinol/β-carotene append `vitaminAKeyDifferencesHtml`; K1/K2 append `vitaminKKeyDifferencesHtml`; general paragraphs (`tooLow` / `enough` / `tooHigh`), then sex-specific (`male` / `female`) notes; `foodSources` / `foodSourceGroups` render into footer `#micro-def-sources` via `setMicroDefFoodSources` / `microDefFoodSourcesHtml` (collapsible `<details open>`). Second arg `returnTo` sets `defModalReturnSources` for **← My food** back nav.
- **`openLongevityDefModal(key)` / `renderLongevityDefBody`** — handles `LONGEVITY_FIELDS` keys, derived keys (`LONGEVITY_DERIVED_DEFS`), and section keys (`LONGEVITY_SECTION_DEFS`); limiting nutrients swap heading order; food sources use the same bottom dock.
...
- **TDEE calculator** — `#tdee-calculator-modal`: Mifflin–St Jeor BMR × activity factor; resistance (days/week or weekly heavy/light sets) + optional cardio (`#tdee-calc-cardio-rows`: one or more session groups with days/week, minutes/session, intensity light→brisk/incline→moderate→hard→vigorous; 30 min = baseline; **Add another cardio** / remove on rows after the first). Intensity helpers under cardio open `#tdee-hr-zones-modal` (Karvonen: age + resting HR → bpm ranges; working HR highlights Light→Vigorous) and `#tdee-rpe-modal` (talk test / RPE → intensity). Inputs persist in settings `tdeeCalc` as `cardioSessions[]` (legacy single-session fields migrate on load); **Reset** clears `tdeeCalc` to null and blanks the form; **Use this value** writes settings `tdee` (+ body weight when set).
- **Week summary compare** — `renderWeekSummary` uses `weekAverageDayCount()` (Mon–today on the current week; otherwise 7) for day average and TDEE×days vs intake; deficit/surplus label, cal so far or cal/week, ~lb/week at current pace (`3500` rule). Explain: `#tdee-hint-modal`.
...
**Word wrap toggle** — `#day-word-wrap-toggle` calls `setDayWordWrapEnabled`, persisted as settings `dayWordWrap` (default on); off adds body class `day-word-wrap-off` so `.day__input` / `.day__backdrop` use `white-space: pre` (horizontal scroll) instead of `pre-wrap`.
...
## Sticky filters section carousel

At `max-width: 900px`, micro / longevity sticky filter groups become a one-section-at-a-time horizontal carousel (CSS scroll-snap on `[data-sticky-filters-track]`).

- **State** — `stickyFiltersCarouselState` (`WeakMap` of track → `{index}`), `stickyFiltersCarouselMq` (`matchMedia("(max-width: 900px)")`), `isStickyFiltersCarouselActive()`.
...
**Guided DOM** — `ensureDayGuidedEl` injects `.day__guided` (list + **Add food** / **Duplicate** + **Add Others** + **Rearrange**). Rearrange shows a drag handle on each line; a short click on the handle (press and release within `GUIDED_HANDLE_SELECT_MS`) toggles multi-select (`.day__food-item--selected`); dragging any row moves the selected set plus the dragged row as a block (`reorderGuidedEntries` / `moveGuidedEntriesToDay`) with before/after/end drop indicators, including onto another day’s food column. After a drag ends, `#guided-finish-popover` (**Finish Rearrange?**) appears at the pointer; click it to exit that day’s rearrange, or it hides if the pointer moves past `GUIDED_FINISH_MOVE_PX` (and is not over the popover) or the page/list scrolls. While Rearrange is on and nothing is selected, **Add food** is disabled (grayed). When at least one line is selected, **Duplicate** replaces **Add food** in that column (`duplicateGuidedSelectedLines` inserts copies after the last selected line, then exits Rearrange and scrolls the new lines into view in `.day__guided-scroll`). **Remove** (×): a short click asks confirm and deletes that line. Click-and-hold × (~`GUIDED_REMOVE_HOLD_MS`) enters multi-select delete (`.day__guided--remove-select`): that line’s × stays visually on (`.day__food-item-remove--on` / `.day__food-item--remove-selected`); while in this mode, clicking × on any line toggles selection. Footer **Confirm Deletion** (dominant, above Cancel; `[data-action="confirm-guided-remove-select"]`) deletes the selected lines and exits the mode. **Cancel** (`[data-action="cancel-guided-remove-select"]`) exits without deleting. Escape also cancels. Still persists as newline text.

**Add food modal** — `#add-food-modal`: search (`addFoodSearchMatches` / `foodSuggestMatches` + saved meal names, or A–Z browse with meals first when empty), paginated results (`ADD_FOOD_PAGE_SIZE` = 25; `#add-food-pagination` Prev/Next), select definition or a **Meal** (expands its foods into the selected list with stored servings), set servings. Footer is a split **Add to day** / chevron (`#add-food-submit-split`): default adds to the day (`appendDayFoodLine`); **Add to meal** opens stacked `#add-food-meal-picker-modal` to pick an existing saved meal, create a named meal, or **Open Saved meals**. Same modal can target a saved meal (`openAddFoodModalForMeal`) and lock the footer to **Add to meal** (no chevron). When one or more foods are selected, footer **Calculator** (`#add-food-calc-toggle`) toggles a Mac-style four-function pad (`#add-food-calculator`) that expands to the right of the panel (`.add-food-modal--calc-open` on `#add-food-modal`; stage `.add-food-modal__stage` recenters so the panel shifts left). Collapse via the same link, the calculator’s close control, or Escape (Escape collapses the calculator first, then the modal). Manage saved meals from **Saved meals** (`#dashboard-meals-jump` shortcut **G** / `#week-saved-meals-open`; sheet title **Food Sources (Groups)**). Escape / backdrop closes (`closeAddFoodModal` in modal Escape stack + `updateBodyModalOpen`).
...
- **Placement** — `positionDaySuggest` / `daySuggestCaretRect`: prefer below the caret (current line stays visible). Only flip above when there is not enough room below. Tips get an explicit content height (`.day__suggest--tip`). Food lists pin to the remaining editor area (`top` below caret + `bottom: 0`, or `top: 0` + `bottom` above caret) so the panel fills the rest of the textbox; `.day__suggest-list` scrolls inside. Never covers the typing line. Editing backdrop uses `opacity: 0` (not `visibility: hidden`) so caret metrics work.
...
| Add study-only reference | `STUDY_MIN_MICRO_REFS` (aim target, `limiting: false`) or `STUDY_MAX_MICRO_REFS` (ceiling, `limiting: true`) + `TARGET_REF_POPOVER_DETAILS` citation; or `NO_STANDALONE_REF_MICRO_KEYS` (unscored breakdown, e.g. β-carotene, K1/K2/MK-4/MK-7, or bundled like cysteine); surfaced by `microNutrientTargetPct` (priority: FDA DV → IOM bw min → study min → study max → no ref) |
| Add vitamin A subform | `MICRO_FIELDS` keys (`vitaminARetinol`, `vitaminABetaCarotene`); retinol → `STUDY_MAX_MICRO_REFS` (IOM UL); β-carotene → `NO_STANDALONE_REF_MICRO_KEYS`; explain copy in `vitaminAKeyDifferencesHtml` |
...
//   initDaysCarousel → initStickyFiltersCarousel → initLongevityNav → initTargetRefPopoverEvents
