...
│   │   ├── #dashboard-micro-sticky (title, View, Daily Targets, sticky Filter +
│   │   │     condition/status filters row, Highlight/daily-intake/acute, close)
│   │   ├── intro + Ask AI
...
├── .week__days
│   ├── .week__days-favorite-week (#week-nav-favorite + #diary-search-open)
...
│           ├── .day__guided (default; list + Add food / Duplicate + Rearrange; injected by JS)
...
**`.week__nav`** — sits directly above the days toolbar (not above the dashboard). Labeled **Previous week** / **Next week** buttons, clickable range `#week-nav-label` (opens `#week-jump-modal` calendar + typed date → jump to that week), **This week** (`#week-nav-this`), **Favorites** (`#favorites-open` → right slide-in `#favorites-sidebar`), and **Saved meals** (`#week-saved-meals-open` → same `#meals-panel` as `#dashboard-meals-jump`). Prev disables at the earliest diary week (Mon–Sun containing `2026-05-01`). Persistent diary, food-definition, and favorite writes show the transient fixed `#save-toast`; there is no permanent save label in the navigation. Hidden in print / print-preview.
...
**`.dashboard__toggles`** — `Food Sources` (`#dashboard-food-sources-open` → `#food-sources-modal`), `Week total` (`#dashboard-week-toggle`), `Micro requirements` (`#dashboard-micro-toggle`), `Longevity` (`#dashboard-longevity-toggle`) on the row below; shared `.dashboard__toggle` / `--open`.
...
- **`#dashboard-micro-sticky`** (`.dashboard__micro-sticky`) — sticky top chrome: `.dashboard__micro-sticky-title` (**Micro Requirements**), View + **Daily Targets**, `.dashboard__micro-sticky-filters` row (inline S/E·A/E + nutrient search + condition/status filters), Highlight / Poor storage / One-day excess disclosures, `#dashboard-micro-close`.
- **Sticky options** (shared pattern with longevity nav):
  ...
  - **Micro sticky filters row** (`.dashboard__micro-sticky-filters`) — inline S/E + A/E checkboxes (`data-sticky-filter`), clear × (`data-sticky-filter-clear`), nutrient search (`#micro-nutrient-filter-input` + chips), condition Filter dropdown + Zero / Red-or-zero / Green status filters.
  - Longevity sticky filters row (`.dashboard__longevity-sticky-filters`) — inline daily/S/E/A/E checkboxes + nutrient search/chips (no Quick Add presets); shares `filterStickyNutrientKeys` / sticky icon filter state with micro.
...
- `.dashboard__micro-condition-wrap` (in sticky filters row) — three dropdowns: **Nutrition Intake**, **Conditions**, **Status** (Zero / Red-or-zero / Green / Excess) + per-dropdown clear ×.
...
- `#micro-tip-hair-loss`, `#micro-tip-hair-loss-herbs`, `#micro-tip-hair-loss-prescriptions` — shown when **Hair loss** condition filter is active.
- `#micro-tip-common-deficiencies` — stays visible for **American Common Deficiencies** (and when no condition filter is active); points users at the sticky condition Filter / nutrient search.
- `#micro-tip-fat-soluble` — stays visible for **Fat-soluble vitamins** condition focus.
- Tip asides (`.dashboard__micro-tip`, `.dashboard__longevity-processed-note`) — click body to collapse (`data-tip-collapsed`); **Read more** / links still open modals. Micro tips: grouped `.dashboard__tips-reopen` (`Tips (n)`). Longevity tips: in-place `.dashboard__tips-reopen[data-tip-reopen-for]` (`Tips (1)`) at each tip. State: `collapsedTipIds` + `syncAllDismissibleTips` / `handleDismissibleTipClick`.
...
- **`#dashboard-longevity-nav`** — sticky: `.dashboard__longevity-nav-title` (**Longevity**), Highlight / Poor storage / One-day excess, `.dashboard__longevity-sticky-filters` (daily/S/E/A/E + nutrient search), `#dashboard-longevity-close`, topic carousel + All topics list.
...
**`.keywords__filter`** — search box `#keywords-search` (`type="search"`) + `#keywords-search-clear` (× inside the field, hidden when empty); filters by food-name substring. **Categories** button `#keywords-category-open` opens `#keywords-category-modal`; active category shows `#keywords-category-chip` + `#keywords-category-clear` (× exits category filter).
...
**Header** — `.week__header-actions` (top-right): `#auth-logged-out` shows **Log in** (`#auth-login-open`) + **Sign up** (`#auth-signup-open`); `#auth-logged-in` shows `#auth-user-email` + **Log out** (`#auth-logout`). `syncAuthUi` toggles which block is visible. At `max-width: 720px`, `.week__header` stacks (title/hint full width, then actions full-width right-aligned) so a long email cannot crush `.week__hint` into a one-word column; email ellipsis tightens further at `max-width: 520px`.
...
- **Add food modal** (`#add-food-modal`) — `.add-food-modal__stage` wraps the wide panel plus `#add-food-calculator`. With foods selected, footer **Calculator** (`#add-food-calc-toggle`) toggles `add-food-modal--calc-open`: a Mac-style pad expands to the right and the centered stage shifts the panel left. Collapse with the same link, the calculator close control, or Escape (before closing the modal).
...
- **Definition modal** (`#micro-def-modal`) — shared by micro + longevity “explain”; `.modal__header--with-tools` with `#micro-def-fullscreen-toggle`; `#micro-def-modal-back` (**← My food**, hidden unless opened from sources modal); body `.micro-def__body`; food sources `#micro-def-sources` / `.micro-def__sources-dock` live in `#micro-def-modal-footer` (flex-grow left, collapsible `<details>`); `#micro-def-modal-done` stays shrink-to-fit on the right.
...
- **Food Sources modal** (`#food-sources-modal`) — grocery-oriented table from precomputed `definitions-food-sources.json` (built by `scripts/build-food-sources.js` from micro/longevity `foodSources` / `foodSourceGroups` + `samples/definitions-food.json`); falls back to live matching if the JSON is missing; sortable Food / Nutrient columns; `#food-sources-filter`, `#food-sources-body`, fullscreen toggle. Regenerate the JSON after changing sample foods or food-source labels.
...
- **TDEE calculator** (`#tdee-calculator-modal`) — Mifflin–St Jeor inputs, resistance/cardio activity, `#tdee-calc-result`, Apply/Cancel; cardio intensity helpers open `#tdee-hr-zones-modal` / `#tdee-rpe-modal`.
- **TDEE heart-rate zones** (`#tdee-hr-zones-modal`) — Karvonen zones from age + resting HR; working HR highlights matching intensity row; `data-action="open-tdee-hr-zones-modal"`.
- **TDEE talk test / RPE** (`#tdee-rpe-modal`) — talk/breath cues → RPE 1–10 → intensity; `data-action="open-tdee-rpe-modal"`.
...
- **Micro-gaps modal** (`#micro-gaps-modal`) — mode radios (eat any food vs eat more of what I ate); any-food: preference select + free-text; re-eat: weeks lookback + expandable food checkboxes; prompt preview / copy / open (ChatGPT/Claude buttons carry brand icons).
...
- **Import all meals** (`#import-all-meals-modal`) — paste JSON or `#import-all-meals-file-btn` (**Import JSON file**) to fill `#import-all-meals-json`; apply with `#import-all-meals-apply`.
...
- Daily intake / acute toxicity / target-ref / guided food-item popovers `z-index: 120` (fixed tooltips, not modals).
- Micros / longevity tooltip on button is above the row (cells use `overflow: visible` so it isn’t clipped).

## CSS naming convention

BEM-like blocks: `.week__`, `.week__auth*`, `.day__`, `.dashboard__`, `.dashboard__nutrient-filter*`, `.keywords__`, `.settings-modal__`, `.tdee-calc__`, `.macro-split-carousel`, `.micro-sources-modal__`, `.food-sources-modal__`, `.import-ai-`, `.micro-gaps-modal__`, `.micro-def__` (incl. footer `.micro-def__sources-dock` collapsible food sources), `.micro-tip-modal__`, `.longevity-form`, `.starter-guide__`, `.favorites-sidebar__`, `.favorites-manage-*`, `.modal__`. Longevity Level bars: `.dashboard__longevity-bar-wrap`, `.dashboard__longevity-bar-notch`, `.dashboard__longevity-bar-notch-popover`.
...
- TDEE: `tdee-calculator-modal`, `tdee-calculator-apply`, `tdee-calculator-cancel`, `tdee-calc-*`, `tdee-hr-zones-modal`, `tdee-hr-zones-modal-done`, `tdee-hr-zones-age`, `tdee-hr-zones-resting`, `tdee-hr-zones-working`, `tdee-hr-zones-status`, `tdee-hr-zones-tbody`, `tdee-rpe-modal`, `tdee-rpe-modal-done`, `tdee-hint-modal`, `tdee-hint-modal-done`
...
- Sources: `food-sources-modal`, `food-sources-modal-title`, `food-sources-body`, `food-sources-filter`, `food-sources-modal-done`, `food-sources-fullscreen-toggle`, `micro-sources-modal`, `micro-sources-modal-title`, `micro-sources-body`, `micro-sources-scope`, `micro-sources-modal-done`, `micro-sources-fullscreen-toggle`, `longevity-sources-modal`, `longevity-sources-modal-title`, `longevity-sources-body`, `longevity-sources-modal-done`, `longevity-sources-fullscreen-toggle`
- Add food / saved meals: `add-food-modal`, `add-food-submit-split`, `add-food-submit`, `add-food-submit-menu-toggle`, `add-food-submit-menu`, `add-food-calc-toggle`, `add-food-calculator`, `add-food-calc-display`, `add-food-calc-clear`, `add-food-meal-picker-modal`, `add-food-meal-picker-list`, `add-food-meal-picker-name`, `add-food-meal-picker-create`, `add-food-meal-picker-open-all`, `meal-name-modal`, `meals-panel`, `meals-new`, `dashboard-meals-jump`
- Day / week / favorites: `mon` … `sun`, `week-nav-prev`, `week-nav-next`, `week-nav-label`, `week-nav-label-text`, `week-nav-this`, `week-nav-favorite`, `week-saved-meals-open`, `favorites-open`, `favorites-sidebar`, `favorites-sidebar-title`, `favorites-sidebar-hint`, `favorites-list`, `favorites-manage-toggle`, `favorites-manage-list`, `favorites-empty`, `favorite-edit-modal`, `favorite-edit-modal-title`, `favorite-edit-modal-hint`, `favorite-edit-name`, `favorite-edit-description`, `favorite-edit-error`, `favorite-edit-save`, `favorite-edit-cancel`, `days-carousel-current`, `week-jump-modal`, `week-jump-date`, `week-jump-typed`, `week-jump-preview`, `week-jump-error`, `week-jump-apply`, `week-jump-cancel`, `copy-date-modal`, `copy-date-modal-title`, `copy-date-modal-hint`, `copy-date-input`, `copy-date-modal-error`, `copy-date-apply`, `copy-date-cancel`, `day-highlights-toggle`, `day-word-wrap-toggle`, `day-food-notes`, `day-food-notes-labels`, `day-food-notes-popover`, `day-food-item-popover`, `day-unmatched-lines`, `export-all-meals`, `import-all-meals`, `import-all-meals-modal`, `import-all-meals-file-btn`, `import-all-meals-file`, `clear-all-days`
- Dashboard: `dashboard-grid`, `dashboard-print`, `week-summary`, `dashboard-food-sources-open`, `dashboard-food-definitions-jump`, `dashboard-meals-jump`, `dashboard-week-toggle`, `dashboard-micro-toggle`, `dashboard-micro-panel`, `dashboard-micro-list`, `dashboard-micro-daily-grid`, `micro-daily-intake-popover`, `target-ref-popover`, `target-ref-popover-text`, `dashboard-micro-view-weekly`, `dashboard-micro-view-daily`, `dashboard-micro-dv-toggle`, `dashboard-micro-hint`, `dashboard-micro-hint-text`, `micro-tip-caffeine`, `micro-tip-cataracts`, `micro-tip-hair-loss`, `micro-tip-hair-loss-herbs`, `micro-tip-hair-loss-prescriptions`, `micro-tip-common-deficiencies`, `micro-tip-fat-soluble`, `dashboard-micro-condition-toggle`, `dashboard-micro-condition-list`, `dashboard-micro-condition-label`, `dashboard-micro-condition-clear`, `micro-nutrient-filter-panel`, `micro-nutrient-filter-input`, `micro-nutrient-filter-suggest`, `longevity-nutrient-filter-panel`, `longevity-nutrient-filter-input`, `longevity-nutrient-filter-suggest`, `micro-gaps-ai-open`, `health-timeline-ai-open`
...
- Micro gaps: `micro-gaps-modal`, `micro-gaps-any-panel`, `micro-gaps-reeat-panel`, `micro-gaps-preference`, `micro-gaps-additional`, `micro-gaps-weeks`, `micro-gaps-foods-details`, `micro-gaps-foods-count`, `micro-gaps-foods-empty`, `micro-gaps-foods-list`, `micro-gaps-ai-preview`, `micro-gaps-ai-copy`, `micro-gaps-open-chatgpt`, `micro-gaps-open-claude`, `micro-gaps-modal-done`
...
- Definitions: `micro-def-modal`, `micro-def-modal-title`, `micro-def-body`, `micro-def-sources`, `micro-def-modal-done`, `micro-def-modal-back`, `micro-def-fullscreen-toggle`
...
- Import: `import-modal`, `import-json`, `import-ai-*`, `import-all-modal`, `import-all-json`, `import-all-meals-json`, `import-all-meals-file-btn`, `import-all-meals-file`, `micro-modal`, `micro-form`
