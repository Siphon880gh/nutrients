...
│  Quiz mode → selected portions → paste-ready CSV;       │
...
Micros and longevity nutrients affect **storage and UI** (button codes, import JSON) and **dashboard panels** (micro % DV vs demographic, longevity % DV vs longevity targets). Macros-only cards still omit micros/longevity; expand **Micro requirements** or **Longevity** on the dashboard for average daily values (`weekAverageDayCount()`: ÷7 for past/future weeks; Mon–today for the current week).
...
- `MICRO_CONDITION_FOCUS` — condition ids (`adhd`, `americanCommonDeficiencies`, `fatSolubleVitamins`, `bVitamins`, `anemia`, `antiAging`, `bowelMovementsAltered`, `cataractsPrevention`, `coffeeTeaUser`, `hairLoss`) → nutrient key lists (+ optional `longevityNutrients`); filters micro panel rows and adds condition notes in def modals. **American Common Deficiencies**, **Fat-soluble vitamins**, and **B vitamins** sit under the **Nutrition Intake** group in the condition dropdown; tips `#micro-tip-common-deficiencies` / `#micro-tip-fat-soluble` / `#micro-tip-b-vitamins` stay visible for those focuses.
...
5. `.week__highlight-bar` — **Advanced** (`#day-entry-advanced-toggle`) + **Quiz mode** (`#week-quiz-open` → `#week-quiz-modal` paste-ready flash-card CSV); **Food visibility**: `#day-highlights-toggle` (pen) + `#day-word-wrap-toggle` (wrap; body `day-word-wrap-off` when off), both persisted; `#day-food-notes` (regex-driven notes + popover)
...
- **Day-meal comments:** lines starting with `//` or `#` (`isCommentDayMealLine`) are ignored for totals / unmatched / food notes; empty-line tip offers `//`, `#`, and divider `---` choices ([core doc](./AGENTS_CODE_REFERENCE-core.md)).
