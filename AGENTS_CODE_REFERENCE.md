...
When context is tight: read **this file** first, then open the one feature file you need. Full logic lives in `app.js` (~13,500 lines) — load it whole only when editing behavior, otherwise navigate by the function names listed in the companion docs.
...
```text
nutrients/
├── index.html                      (~1500 lines) Page structure, modals, starter guide, static day editors
├── styles.css                      (~5500 lines) Layout, dashboard, table, modals, starter guide, responsive, print
├── app.js                          (~13,500 lines) All application logic (IIFE)
...
1. **Boot** (end of `app.js`): `loadAppConfig()` → in parallel `loadMicroDefinitions()` + `loadLongevityDefinitions()` + `loadFoodNotesDefinitions()` → `boot()`: `loadFoodDefinitions()` → `loadKeywordReorderOpen()` → `loadKeywordCaloriesOpen()` → `loadKeywordsPageSize()` → `loadDayNotes()` → `loadDayHighlightsPreference()` → `loadDayEditorHeight()` → `loadDemographic()` → `loadTdee()` → `loadBodyWeight()` → `renderDemographicUi()` → `syncSettingsTdeeInput()` / `syncSettingsWeightInput()` → `renderKeywords()` → `refreshAll()` → `maybeShowStarterGuideImportStep()` (empty food list only). Listeners are bound earlier in the same closure (`bindDay` per textarea, `bindDayEditorResize` on the week grid, `initDayFoodNotesEvents` + `initTargetRefPopoverEvents` once).
...
- `MICRO_FIELDS` — `{ key, label, unit, code }` for the core micros (now includes `fiber`, `solubleFiber` (`sf`), `insolubleFiber` (`if`)); `code` drives button label (`f`, `na`, `b12`, `b1`, …).
...
- `STUDY_MAX_MICRO_REFS` — study-derived ceilings (`arginine`, `glutamine`, `taurine`) for nutrients with no FDA/IOM daily value. `NO_STANDALONE_REF_MICRO_KEYS` — nutrients shown unscored (`cysteine`, `glycine`, `proline`).
