...
5. `.week__highlight-bar` — **Food visibility**: `#day-highlights-toggle` (pen) + `#day-word-wrap-toggle` (wrap; body `day-word-wrap-off` when off), both persisted; `#day-food-notes` (regex-driven notes + popover)
...
- **Food-name suggestions:** `updateDaySuggest` / `positionDaySuggest` / `foodSuggestMatches`; places `.day__suggest` below the caret, or `.day__suggest--above` (pinned to editor top) when the caret is in the lower half so the popover does not cover typing ([core doc](./AGENTS_CODE_REFERENCE-core.md)).
