...
2. `.dashboard` — `#dashboard-grid` (today’s card gets `.dashboard__card--today` only when viewing the current week; cards show `.dashboard__date`); toggles `#dashboard-print`, `#dashboard-week-toggle` (`#week-summary`), `#dashboard-micro-toggle` (`#dashboard-micro-panel`), `#dashboard-longevity-toggle` (`#dashboard-longevity-panel`)
...
3. `.week__nav` — **Previous week** / clickable range `#week-nav-label` (opens `#week-jump-modal`) / **Next week** / **This week**; earliest week is the Mon–Sun containing **2026-05-01** (`EARLIEST_DIARY_DATE`)
...
7. `.week__grid` — seven `.day` columns; today’s column gets `.day--today` when viewing this week. Each `.day__head` has label + `.day__date` (`M/D/YY`) + **Copy** dropdown (week→this week / custom week / date→custom/today/yesterday/tomorrow) / Clear. Each `.day__editor` (backdrop + textarea + optional `.day__suggest`). Modes: **editing** / **viewing** / **plain**; shared vertical resize.
...
- **Multi-week diary:** `dayMealsByDate` + `viewedWeekStart`; earliest week containing `EARLIEST_DIARY_DATE` (`2026-05-01`); `setViewedWeekStart` / `stepViewedWeek` / `goToThisWeek`; date labels via `updateDayDateLabels` / `formatDayDateLabel`; Copy menu via `handleDayCopyAction` / `#copy-date-modal` ([core doc](./AGENTS_CODE_REFERENCE-core.md)).
