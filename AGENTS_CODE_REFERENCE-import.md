...
- AI schema example must stay aligned with `MICRO_ALL_FIELDS` (core + extended + amino acids, including vitamin A subforms `vitaminARetinol` / `vitaminABetaCarotene` and vitamin K subforms `vitaminK1` / `vitaminK2` / `vitaminK2MK4` / `vitaminK2MK7`) + `LONGEVITY_FIELDS` keys (`plantSterols`, `creatine`, `carotenoids`, `alphaLipoicAcid`, `glutathione`, …) (`jsonSchemaExample`, `nutrientListForPrompt`). Multivitamin absorption + fat-soluble fat-assumption rules live in `buildAiPromptFromPortion`.
...
- **Carotenoids:** may be omitted when `vitaminA` or `vitaminABetaCarotene` is present — dashboard estimates mg via `resolveLongevityValue` (prefers β-carotene mcg ÷ 1000, else vitamin A RAE heuristic); explicit `longevity.carotenoids` (mg) overrides.
